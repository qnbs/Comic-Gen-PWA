...
const UploadPanel: React.FC = () => {
  ...
  const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (selectedFile) {
      if (selectedFile.type === 'text/plain' && selectedFile.size <= 50000) {
        // 50KB limit
  ...
  const handleSubmit = React.useCallback(() => {
    ...
    reader.onload = (e) => {
      ...
      dispatch(startAnalysis({ text, language }));
    };
    reader.onerror = () => {
      setError(t('importer.readFileError'));
    };
    reader.readAsText(file);
  }, [file, dispatch, t, language]);
  ...
  return (
    <div className="flex flex-col items-center">
      <div
        ...
        className={`w-full p-6 sm:p-10 border-2 border-dashed rounded-lg cursor-pointer transition-colors duration-300 ${
          isDragging
...
const UrlPanel: React.FC = () => {
  ...
  return (
    <div className="flex flex-col items-center">
      ...
      />
      <button
        onClick={() => dispatch(startAnalysis({ text, language }))}
        disabled={!text.trim()}
        className="mt-6 w-full py-3 px-6 bg-indigo-600 rounded-lg text-lg font-bold text-white transition-all duration-300 shadow-lg hover:bg-indigo-700 disabled:bg-gray-500 dark:disabled:bg-gray-600 disabled:cursor-not-allowed disabled:opacity-50"
      >
        {t('importer.generate')}
...
const BrowsePanel: React.FC = () => {
  ...
  const dispatch = useAppDispatch();
  const { language } = useAppSelector((state) => state.ui);
  const currentBooks = publicBooks[language] || publicBooks.en;

  return (
    <div>
      <p className="text-gray-600 dark:text-gray-400 mb-6 text-center">
        {t('importer.browseDescription')}
      </p>
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 max-h-[50vh] overflow-y-auto pr-2">
        {currentBooks.map((book) => (
          <div
            key={book.title}
            onClick={() =>
              dispatch(startAnalysis({ text: book.text, language }))
            }
            className="bg-white dark:bg-gray-800 p-4 rounded-lg border border-gray-300 dark:border-gray-700 cursor-pointer hover:border-indigo-500 hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-all duration-300 flex items-start gap-4"
          >
            <img
              src={book.coverImageUrl}
              alt={`Cover for ${book.title}`}
              className="w-20 h-28 object-cover rounded-md flex-shrink-0 bg-gray-300 dark:bg-gray-700"
            />
            <div className="flex-grow">
              <h3 className="text-lg font-bold text-indigo-600 dark:text-indigo-300">
                {book.title}
              </h3>
...
const BookImporter: React.FC = () => {
  const [mode, setMode] = React.useState<ImportMode>('browse');
  const { t } = useTranslation();
  const dispatch = useAppDispatch();
  ...
  const handleDiscard = () => {
    if (window.confirm(t('app.resume.discardConfirmation'))) {
  ...
  }> = ({ activeMode, targetMode, children, icon }) => (
    <button
      id={`${targetMode}-tab`}
      role="tab"
      aria-selected={activeMode === targetMode}
      aria-controls={`${targetMode}-panel`}
      onClick={() => setMode(targetMode)}
      className={`flex-1 flex items-center justify-center gap-2 px-4 py-2 text-sm font-medium rounded-t-lg border-b-2 transition-colors focus:outline-none focus-visible:ring-2 focus-visible:ring-indigo-500 focus-visible:ring-offset-2 dark:focus-visible:ring-offset-gray-800 ${
    ...
    <div className="w-full max-w-2xl mx-auto bg-white/50 dark:bg-gray-800/50 rounded-2xl border border-gray-300 dark:border-gray-700 shadow-2xl overflow-hidden">
      {savedProgress && (
        <div className="p-4 bg-indigo-50 dark:bg-indigo-900/50 border-b border-indigo-200 dark:border-indigo-700 text-center">
          <h3 className="font-bold text-lg text-indigo-700 dark:text-indigo-300 flex items-center justify-center gap-2">
            <SaveIcon className="w-5 h-5" />
            {t('app.resume.title')}
          </h3>
          <p className="text-sm text-gray-700 dark:text-gray-300 mt-1 mb-4">
            {t('app.resume.body', {
              date: new Date(savedProgress.timestamp).toLocaleString(),
            })}
          </p>
          <div className="flex justify-center gap-4">
            <button
              onClick={() => dispatch(resumeSession())}
              className="px-5 py-2 bg-indigo-600 hover:bg-indigo-700 rounded-lg font-semibold text-white transition-colors"
            >
              {t('app.resume.resumeButton')}
            </button>
            <button
              onClick={handleDiscard}
              className="px-5 py-2 bg-gray-500 hover:bg-gray-600 text-white rounded-lg font-semibold transition-colors flex items-center gap-2"
            >
              <TrashIcon className="w-4 h-4" />
              {t('app.resume.discardButton')}
            </button>
          </div>
        </div>
      )}
