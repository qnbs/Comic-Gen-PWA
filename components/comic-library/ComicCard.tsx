...
const ComicCard: React.FC<ComicCardProps> = ({ comic, onLoadComic }) => {
  const { t } = useTranslation();
  const { thumbnailUrls, selectedComics, handleSelectComic, handleSingleDelete } =
    useComicLibraryContext();

  const isSelected = selectedComics.includes(comic.id);
  ...
  return (
    <div
      ...
      onKeyDown={(e: React.KeyboardEvent<HTMLDivElement>) =>
        (e.key === 'Enter' || e.key === ' ') && handleSelectComic(comic.id)
      }
    >
      <div
      ...
      <div className="absolute inset-0 bg-gradient-to-t from-black/80 via-black/40 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex flex-col justify-end p-4">
        ...
        <div className="mt-4 flex gap-2">
          <button
            ...
            aria-label={`${t('comicLibrary.load')} ${comic.title}`}
          >
