...
const LibraryFooter: React.FC = () => {
    ...
    const { selectedProjects, clearSelection, handleBulkDelete } = useComicLibraryContext();

    return (
        <footer className={`mt-auto pt-4 border-t border-gray-300 dark:border-gray-700 transition-all duration-300 ${selectedProjects.length > 0 ? 'opacity-100' : 'opacity-0 pointer-events-none'}`}>
            <div className="flex items-center justify-between">
                <p className="font-semibold text-indigo-700 dark:text-indigo-300">
                    {t('comicLibrary.selectedCount', { count: selectedProjects.length })}
                </p>
